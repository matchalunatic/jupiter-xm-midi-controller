...
from jupiterlib import (
    build_roland_rq1_message,
    get_sysex_data,
    validate_rq1_and_get_value,
    build_parameter_address,
)
...
def repl():
    while True:
        ...
        if not addr:
            addr_to_hexbytes = build_parameter_address(
                ZCORE_1, ZC_TONE_PARTIAL_1, TONE_PARTIAL_CUTOFF_FREQ
            )
            addr_to_hexbytes = tuple(
                a + b for a, b in zip(addr_to_hexbytes, [0, 0, 0, 0])
            )
...
def mainloop_b():
    ...
    for idx, elem in enumerate(
        range(start_search, start_search + 0x1_00_00_00, striding)
    ):
        ...
        if len(sysex_state["strings"]) > len_of_strings:
            print(
                "found a new string",
                sysex_state["strings"][len_of_strings],
                "at ",
                lhex(elem, 8),
                "\t",
                bin(elem),
...
def mainloop_c():
    ...
    while a < last_address:
        ...
        for idx, el in enumerate(rev):
            ...
            if overflow:
                print(
                    f"bit {idx + 1}/{len(rev)} overflows: {hex(el)}. Truncate to {hex(seven_bit_val)} and set overflow {overflow}"
                )
        if overflow > 0:
